...
pub fn initial_watch_cancels_stale_run_and_publishes_saved_source_test() {
  kangaroo.serial()
    ...
    0 -> {
      let assert Ok(handle) =
        process.start(
          ...
          [],
          // Compile time is outside the save-detection budget and can
          // be substantial on a cold Windows runner. Keep the
          // coordinator alive while this functional test observes all
          // generations; the enclosing test retains a separate bound.
          90_000,
        )
      ...
      let #(configured_roots, config_output) = case completed_latest {
        ...
        True -> {
          ...
          schedule_replace(config_path, config_source, config_replacement, 100)
          await_output(handle, "1 passed, 0 failed", sys.now_ms(), 30_000, "")
        }
      }
      let #(observed_dynamic_root, root_output) = case configured_roots {
        ...
        True -> {
          ...
          schedule_replace(extra_path, extra_source, extra_source <> "\n", 100)
          await_output(
            handle,
      ...
      let #(settled_dynamic_root, settled_output) = case observed_dynamic_root {
