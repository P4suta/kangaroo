...
pub fn watch_plan_extracts_only_included_gleam_modules_test() {
  let snapshot =
    dict.from_list([
  ...
  assert watch_plan.sources(snapshot, ["test/generated/**"])
    == [
      #("src/app.gleam", "pub fn app() { Nil }"),
      #("test/app_test.gleam", "pub fn app_test() { Nil }"),
    ]
...
pub fn watch_plan_requests_full_run_for_config_and_ffi_changes_test() {
  ...
  let assert Ok(ffi_refresh) =
    watch_plan.refresh(state, initial_sources(), ["test"], ["src/app_ffi.erl"])
